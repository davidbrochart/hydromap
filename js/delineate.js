...
(function() {
  var accDelta, accNeighbors, acc_tiles, addLayer, click_url, dirNeighbors, dir_tiles, do_delineate, do_stream, extend_lineBuffer, find1, getTile, get_url, go_get_dir, layers, map, mapOnClick, mm, mm_back, mx, mx0, mx0_deg, mx1, mxw, my, my0, my0_deg, my1, myw, nbline_max, neighbors_i, neighbors_memsize, p_getTile, p_wait, pack_size, pix_deg, pix_i, pol_i, polyLayers, polygonize, polygons, round, runGen, sample_cb, sample_i, sample_nb, samples, spin_opts, spin_target, spinner, state, tile_width, watershed, watershedLayer, watersheds, x, x_deg, y, y_deg;
  ...
  polygonize = function() {
    ...
    while (!all_pol_done) {
      ...
      if (!all_pol_done) {
        ...
        for (i = k = 0, ref = last_polygon.length - 1; 0 <= ref ? k <= ref : k >= ref; i = 0 <= ref ? ++k : --k) {
          last_polygon[i][0] = mx0_deg + last_polygon[i][0] * pix_deg / 2;
          last_polygon[i][1] = my0_deg - last_polygon[i][1] * pix_deg / 2;
        }
      }
    }
    for (i = l = 0, ref1 = polygon.length - 1; 0 <= ref1 ? l <= ref1 : l >= ref1; i = 0 <= ref1 ? ++l : --l) {
    ...
    req.onload = function() {
      ...
      if (arrayBuffer) {
        ...
        if (type === 'dir') {
          ...
          return cb(new Int32Array(arrayBuffer));
        }
      }
    };
    req.send(null);
  ...
  do_delineate = function*(p) {
    var acc, acc_tile, dir_back, dir_next, dir_tile, done, go_down, i, k, l, latlng, m, n, nb, neighbors_new, o, outletLayer, q, r, reached_upper_ws, ref, ref1, ref2, ref3, ref4, ref5, ref6, ret, s, samples_rev, skip, this_acc, this_accDelta, this_outlet, url;
    latlng = p;
    if (state === 'deliSubBas') {
    ...
    if (state === 'watershed' || state === 'getSubBas') {
    ...
    if (state === 'deliSubBas') {
      if (sample_i === 0) {
        ...
        for (i = l = 0, ref1 = mxw * myw - 1; 0 <= ref1 ? l <= ref1 : l >= ref1; i = 0 <= ref1 ? ++l : --l) {
          mm_back[i] |= mm[i];
        }
      }
    }
    this_outlet = turf.point([x_deg + pix_deg / 2, y_deg - pix_deg / 2]);
    if (state === 'deliSubBas' && sample_i > 0) {
      ...
      for (i = m = 0, ref2 = mxw * myw - 1; 0 <= ref2 ? m <= ref2 : m >= ref2; i = 0 <= ref2 ? ++m : --m) {
        mm[i] = 0;
      }
    }
    pix_i = 0;
    ...
    while (!done) {
      ...
      if (!skip) {
        if (state === 'deliSubBas' && (mm_back[my * mxw + Math.floor(mx / 8)] >> (mx % 8)) & 1 === 1) {
          ...
          if (state === 'getSubBas') {
            ...
            if (this_accDelta >= accDelta && this_acc >= accDelta) {
              acc = this_acc;
      ...
      if (reached_upper_ws || nb === 0) {
        ...
        if (neighbors_i === neighbors_memsize) {
          ...
          for (i = o = 0, ref3 = neighbors_memsize - 1; 0 <= ref3 ? o <= ref3 : o >= ref3; i = 0 <= ref3 ? ++o : --o) {
            neighbors_new[i] = dirNeighbors[i];
          }
          dirNeighbors = neighbors_new;
          neighbors_new = new Int32Array(neighbors_memsize * 2);
          for (i = q = 0, ref4 = neighbors_memsize - 1; 0 <= ref4 ? q <= ref4 : q >= ref4; i = 0 <= ref4 ? ++q : --q) {
      ...
      if (done) {
        if (state === 'watershed') {
          ...
          for (i = r = 0, ref5 = mxw * myw - 1; 0 <= ref5 ? r <= ref5 : r >= ref5; i = 0 <= ref5 ? ++r : --r) {
            mm[i] = mm[i] & ~mm_back[i];
          }
        }
        if (state !== 'getSubBas') {
        ...
        if (state === 'getSubBas') {
          samples_rev = [];
          for (i = s = 0, ref6 = samples.length - 1; 0 <= ref6 ? s <= ref6 : s >= ref6; i = 0 <= ref6 ? ++s : --s) {
  ...
  do_stream = function*(p) {
    var dir_tile, done, i, latlng, ret, source, sourceLayer, stream, streamLayer, url, x_deg_keep, y_deg_keep;
    if (state === 'stream') {
      spinner.spin(spin_target);
    }
    latlng = p;
  ...
  go_get_dir = function(dir, go, first, dir_tile, acc_tile) {
    ...
    if (ret['url'] === 0) {
      ...
      if ((acc_tiles[tile_i] != null) && acc_tiles[tile_i] !== 0) {
  ...
  get_url = function(lat, lon, set_xy) {
    var acc_code, acc_url, dir_code, dir_url, lat0, lat_str, lon0, lon_str, ret, tile_name;
    lat = round(lat, 5);
    lon = round(lon, 5);
    ret = {};
    lat0 = 90;
    ...
    contextmenuItems: [
      {
        ...
        callback: function(e) {
          state = 'watershed';
          return runGen(do_delineate, [e.latlng.lat, e.latlng.lng]);
        }
      }, {
        text: 'Show stream',
        callback: function(e) {
          state = 'stream';
          return runGen(do_stream, [e.latlng.lat, e.latlng.lng]);
        }
      }, '-', {
  ...
  map.on('mousemove', function(e) {
    ...
    return window['flowAcc'].innerHTML = '';
  });
