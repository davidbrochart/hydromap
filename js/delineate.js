    ...
    while (!done) {
      ...
      if (reached_upper_ws || nb === 0) {
        ...
        if (neighbors_i === neighbors_memsize) {
          ...
          for (i = q = 0, ref6 = neighbors_memsize - 1; 0 <= ref6 ? q <= ref6 : q >= ref6; i = 0 <= ref6 ? ++q : --q) {
            neighbors_new[i] = dirNeighbors[i];
          }
          dirNeighbors = neighbors_new;
          neighbors_new = new Int32Array(neighbors_memsize * 2);
          for (i = r = 0, ref7 = neighbors_memsize - 1; 0 <= ref7 ? r <= ref7 : r >= ref7; i = 0 <= ref7 ? ++r : --r) {
      ...
      if (done) {
        if (state === 'watershed') {
          ...
          for (i = s = 0, ref8 = mxw * myw - 1; 0 <= ref8 ? s <= ref8 : s >= ref8; i = 0 <= ref8 ? ++s : --s) {
            mm[i] = mm[i] & ~mm_back[i];
          }
        }
        if (state !== 'getSubBas') {
        ...
        if (state === 'getSubBas') {
          samples_rev = [];
          orders_rev = [];
          for (i = t = 0, ref9 = samples.length - 1; 0 <= ref9 ? t <= ref9 : t >= ref9; i = 0 <= ref9 ? ++t : --t) {
