...
(function() {
  var addPixel, find1, getTile, get_url, go_get_dir, load_tile_and_go_get_dir, map, map_on_click, neighbors, neighbors_i, neighbors_memsize, outlet, pack_size, pix_deg, pix_deg2, pix_i, polyLayers, polygons, processTile, round, run, tile_width, tiles, url, wait, x, x_deg, y, y_deg;
  ...
  addPixel = function() {
    var done_packing, i, j, k, level, ref, ref1, results;
    polygons[pix_i % pack_size] = turf.polygon([[[x_deg - pix_deg2, y_deg - pix_deg2], [x_deg - pix_deg2, y_deg + pix_deg2], [x_deg + pix_deg2, y_deg + pix_deg2], [x_deg + pix_deg2, y_deg - pix_deg2], [x_deg - pix_deg2, y_deg - pix_deg2]]]);
    done_packing = false;
    pix_i += 1;
    i = pix_i;
    level = 1;
    ...
    while (!done_packing) {
      if (i % pack_size === 0) {
        polygons[level * pack_size + (i / pack_size - 1) % pack_size] = turf.merge(turf.featurecollection(polygons.slice(level * pack_size - pack_size, +(level * pack_size - 1) + 1 || 9e9)));
        ...
        for (k = j = ref = level * pack_size - pack_size, ref1 = level * pack_size - 1; ref <= ref1 ? j <= ref1 : j >= ref1; k = ref <= ref1 ? ++j : --j) {
          polygons[k] = 0;
          if (level > 1) {
            if (polyLayers[k - pack_size] != null) {
        ...
        results.push(done_packing = true);
      }
    }
    return results;
  };
  ...
  processTile = function*() {
    var dir_back, dir_next, done, go_down, i, j, l, layer, len, m, nb, neighbors_new, outletLayer, ref, results, skip, watershed, watershedLayer;
    done = false;
    skip = false;
    pix_i = 0;
    polygons = [];
    ...
    while (!done) {
      ...
      if (nb === 0) {
        ...
        if (neighbors_i === neighbors_memsize) {
          ...
          for (i = l = 0, ref = neighbors_memsize - 1; 0 <= ref ? l <= ref : l >= ref; i = 0 <= ref ? ++l : --l) {
      ...
      if (done) {
        for (m = 0, len = polyLayers.length; m < len; m++) {
          layer = polyLayers[m];
