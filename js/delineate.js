...
(function() {
  var find1, getTile, get_url, go_get_dir, load_tile_and_go_get_dir, map, map_on_click, ms, neighbors, neighbors_i, neighbors_memsize, outlet, pix_deg, pix_deg2, pix_nb, polygon1, polygon2, processTile, round, run, tile_width, tiles, url, wait, watershedLayer, x, x_deg, x_pix, y, y_deg, y_pix;
  ...
  getTile = function(url) {
    ...
    req.onload = function() {
      ...
      if (arrayBuffer) {
        console.log('Done!');
        return deferred.resolve(new Uint8Array(arrayBuffer));
      } else {
        return deferred.reject(new Error("Can't do it"));
      }
    };
    req.send(null);
    return deferred.promise;
  ...
  processTile = function*() {
    var dir_back, dir_next, done, go_down, i, j, k, l, m, nb, neighbors_new, outletLayer, pix_i, ref, ref1, ref2, results, skip;
    done = false;
    skip = false;
    pix_i = 0;
    results = [];
    while (!done) {
      if (!skip) {
        ...
        if (pix_i === pix_nb) {
          pix_i = 0;
          for (i = j = 0, ref = pix_nb - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
            polygon1[i] = turf.polygon([[[x_pix[i] - pix_deg2, y_pix[i] - pix_deg2], [x_pix[i] - pix_deg2, y_pix[i] + pix_deg2], [x_pix[i] + pix_deg2, y_pix[i] + pix_deg2], [x_pix[i] + pix_deg2, y_pix[i] - pix_deg2], [x_pix[i] - pix_deg2, y_pix[i] - pix_deg2]]]);
          }
      ...
      if (nb === 0) {
        if (neighbors_i === 0) {
          ...
          if (pix_i === pix_nb) {
            pix_i = 0;
            for (i = l = 0, ref1 = pix_nb - 1; 0 <= ref1 ? l <= ref1 : l >= ref1; i = 0 <= ref1 ? ++l : --l) {
              polygon1[i] = turf.polygon([[[x_pix[i] - pix_deg2, y_pix[i] - pix_deg2], [x_pix[i] - pix_deg2, y_pix[i] + pix_deg2], [x_pix[i] + pix_deg2, y_pix[i] + pix_deg2], [x_pix[i] + pix_deg2, y_pix[i] - pix_deg2], [x_pix[i] - pix_deg2, y_pix[i] - pix_deg2]]]);
            }
            polygon1[pix_nb] = polygon2;
            polygon2 = turf.merge(turf.featurecollection(polygon1));
        ...
        if (neighbors_i === neighbors_memsize) {
          ...
          for (i = m = 0, ref2 = neighbors_memsize - 1; 0 <= ref2 ? m <= ref2 : m >= ref2; i = 0 <= ref2 ? ++m : --m) {
      ...
      if (done) {
        if (pix_i !== 0) {
          polygon1 = (function() {
            var n, ref3, results1;
            results1 = [];
            for (i = n = 0, ref3 = pix_i - 1; 0 <= ref3 ? n <= ref3 : n >= ref3; i = 0 <= ref3 ? ++n : --n) {
              results1.push(turf.polygon([[[x_pix[i] - pix_deg2, y_pix[i] - pix_deg2], [x_pix[i] - pix_deg2, y_pix[i] + pix_deg2], [x_pix[i] + pix_deg2, y_pix[i] + pix_deg2], [x_pix[i] + pix_deg2, y_pix[i] - pix_deg2], [x_pix[i] - pix_deg2, y_pix[i] - pix_deg2]]]));
            }
            return results1;
          })();
        ...
        results.push(watershedLayer.on('click', function(e) {
          var link;
  ...
  go_get_dir = function(dir, go) {
    var x_deg_next, x_next, y_deg_next, y_next;
    x_next = x;
    y_next = y;
    x_deg_next = x_deg;
    y_deg_next = y_deg;
    ...
    if (x_next === -1 || y_next === -1 || x_next === tile_width || y_next === tile_width) {
      return -1;
    } else {
      if (go) {
        x = x_next;
  ...
  load_tile_and_go_get_dir = function*(dir, go) {
    var tile_i, x_deg_next, x_next, y_deg_next, y_next;
    x_next = x;
    y_next = y;
    x_deg_next = x_deg;
    y_deg_next = y_deg;
  ...
  map_on_click = function(evt) {
    ...
    polygon1 = (function() {
      var k, ref1, results;
      results = [];
      for (i = k = 0, ref1 = pix_nb; 0 <= ref1 ? k <= ref1 : k >= ref1; i = 0 <= ref1 ? ++k : --k) {
        results.push(0);
      }
      return results;
    })();
  ...
  get_url = function(lat, lon, set_xy) {
    var lat0, lat_str, lon0, lon_str, ret, tile_code, tile_name;
    lat = round(lat, 5);
    lon = round(lon, 5);
    ret = {};
    lat0 = 90;
