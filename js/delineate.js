...
(function() {
  var addLayer, do_delineate, do_stream, extend_lineBuffer, find1, getTile, get_url, go_get_dir, layers, map, mm, mm_back, mx, mx0, mx0_deg, mx1, mxw, my, my0, my0_deg, my1, myw, nbline_max, neighbors, neighbors_i, neighbors_memsize, outlet, p_getTile, p_wait, pack_size, pix_deg, pix_i, pol_i, polyLayers, polygonize, polygons, round, runGen, sample_cb, sample_i, sample_nb, samples, source_i, sources, spin_opts, spin_target, spinner, state, tile_width, tiles, watershed, watershedLayer, watersheds, x, x_deg, y, y_deg;
  ...
  do_delineate = function*(p) {
    var dir_back, dir_next, done, go_down, i, k, l, latlng, m, n, nb, neighbors_new, o, outletLayer, q, reached_upper_ws, ref, ref1, ref2, ref3, ref4, ret, skip, this_outlet, this_tile, url;
    latlng = p;
    if (state === 'wsOnStr') {
    ...
    while (!done) {
      ...
      if (done) {
        if (state === 'watershed') {
          ...
          for (i = q = 0, ref4 = mxw * myw - 1; 0 <= ref4 ? q <= ref4 : q >= ref4; i = 0 <= ref4 ? ++q : --q) {
            mm[i] = mm[i] & ~mm_back[i];
          }
        }
        if (state === 'getSources') {
  ...
  do_stream = function*(p) {
    var done, i, latlng, ref, ref1, ret, source, sourceLayer, stream, streamLayer, this_tile, url, x_deg_keep, y_deg_keep;
    if (state === 'srcStr') {
    ...
    while (!done) {
      ...
      if (((outlet[0] + pix_deg / 4 > (ref = x_deg + pix_deg / 2) && ref > outlet[0] - pix_deg / 4)) && ((outlet[1] + pix_deg / 4 > (ref1 = y_deg - pix_deg / 2) && ref1 > outlet[1] - pix_deg / 4))) {
        done = true;
      }
    }
