...
(function() {
  ...
  do_delineate = function*(p) {
    var cb, dir_back, dir_next, done, go_down, i, k, l, latlng, m, n, nb, neighbors_new, o, outlet, outletLayer, q, reached_upper_ws, ref, ref1, ref2, ref3, ref4, ret, save, skip, this_tile, url;
    latlng = p[0];
    save = p[1];
    cb = p[2];
    if (save) {
    ...
    while (!done) {
      ...
      if (!skip) {
        if (save && (mm[my * mxw + Math.floor(mx / 8)] >> (mx % 8)) & 1 === 1) {
  ...
  do_stream = function*(p) {
    var cb, done, i, latlng, ret, sample, source, sourceLayer, stream, streamLayer, this_tile, url, x_deg_keep, y_deg_keep;
    latlng = p[0];
    sample = p[1];
    cb = p[2];
    url = get_url(latlng[0], latlng[1], true);
    ...
    contextmenuItems: [
      {
        text: 'Show watershed',
        callback: function(e) {
          return runGen(do_delineate, [[e.latlng.lat, e.latlng.lng], false]);
        }
      }, {
        text: 'Show stream',
        callback: function(e) {
          return runGen(do_stream, [[e.latlng.lat, e.latlng.lng], false]);
        }
      }, '-', {
        text: 'Show watersheds along stream',
        callback: function(e) {
          return runGen(do_stream, [
            [e.latlng.lat, e.latlng.lng], true, function() {
              sample_i = 0;
              return runGen(do_delineate, [
                [0, 0], true, function() {
                  return runGen(do_delineate, [[0, 0], true]);
                }
              ]);
            }
          ]);
